...
<head>
    <meta charset="UTF-8">
    <title>Auxiliar camara pedidos por camion</title>
    <style>
        ...
        .header { border: 1px solid #d1d5db; border-radius: 8px; padding: 8px; margin-bottom: 10px; background: #f9fafb; }
...
<body>
<p class="title">AUXILIAR DE CAMARA - PEDIDOS POR CAMION</p>
<p class="sub">Fecha: {{ $fecha }} | Total pedidos: {{ $pedidos->count() }}</p>

<div class="header">
    <p><span class="label">Camion:</span> {{ $camion->name ?? '-' }} {{ !empty($camion->placa) ? ('(' . $camion->placa . ')') : '' }}</p>
    <p><span class="label">Importe total:</span> {{ number_format((float) $total, 2) }} Bs</p>
</div>
