            ...
            <tbody>
              <template v-for="item in (deudorActual?.items || [])" :key="`${item.tipo}-${item.id}`">
                <tr>
                  ...
                  <td><q-chip dense color="negative" text-color="white">{{ money(item.saldo) }}</q-chip></td>
                  <td>
                    <q-btn-dropdown dense color="primary" no-caps label="Opciones">
                      <q-list>
                        <q-item clickable v-close-popup @click="abrirPagoItem(item, null)">
                          <q-item-section avatar><q-icon name="payments" /></q-item-section>
                ...
                <tr>
                  <td colspan="7" class="bg-grey-1">
                    ...
                    <div v-for="pg in (item.pagos || [])" :key="`pg-${item.tipo}-${item.id}-${pg.id}`" class="row items-center q-col-gutter-sm q-py-xs">
                      ...
                      <div class="col-12 col-md-2">{{ pg.considerar_en_cobranza ? 'Cuenta' : 'No cuenta' }}</div>
                      <div class="col-12 col-md-2">
        ...
        <q-card-section>
          ...
          <div class="row q-col-gutter-sm">
            ...
            <div class="col-12 col-md-6">
              <q-toggle v-model="pagoForm.considerar_en_cobranza" label="Tomar en cuenta en cobranzas" />
            </div>
            <div class="col-12 col-md-6">
...
function abrirPagoRapido(row) {
  if (!row?.items?.length) {
    proxy.$alert.error('No hay documentos de deuda')
    return
  }
  abrirPagoItem(row.items[0], null)
}
...
function abrirPagoItem(item, pago) {
  ...
  pagoForm.value = {
    ...
    considerar_en_cobranza: pago ? !!pago.considerar_en_cobranza : true,
    comprobante: null,
  }
...
async function guardarPago() {
  ...
  try {
    ...
    fd.append('considerar_en_cobranza', pagoForm.value.considerar_en_cobranza ? '1' : '0')
