...
<!--                btn habilitar pedido-->
                <q-btn v-if="cliente.estado!=='CREADO'" @click="habilitarpedido" style="width: 100%"
                       label="Habilitar pedido" icon="check_circle" color="green" :loading="loading"/>
              </div>
            </div>
          </q-card-section>
  ...
  methods: {
    habilitarpedido() {
      this.$q.dialog({
        ...
        message: 'Esta seguro de habilitar el pedido?',
        persistent: true,
        cancel: true,
      }).onOk(() => {
        this.loading = true
        this.$api.post('habilitarpedido', {NroPed: this.cliente.NroPed}).then(res => {
          console.log(res.data)
          this.$q.notify({
            type: 'positive',
            message: 'Pedido habilitado con exito',
          })
    ...
    getListapersonal() {
      this.listapersonales = [{
        CodAut: 0,
        label: 'Todos',
      }]
      this.$api.get('listapersonal').then(res => {
        // this.listapersonales = res.data
        res.data.forEach(r => {
          let d = r
          d.label = r.Nombre1 + ' ' + r.App1
          this.listapersonales.push(d)
        })
      })
    },
    clonarpedido() {
